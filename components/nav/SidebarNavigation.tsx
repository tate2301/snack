import { ArrowPathIcon, DocumentIcon, InboxIcon, PlusIcon } from '@heroicons/react/24/outline';
import CalendarIcon from '../../icons/CalendarIcon';
import useToggle from '../../hooks/useToggle';
import { AppNavigation, NavItem as NavItemType, SidebarToggleProps, Tab } from './types';
				...
				<div className='flex flex-col gap-2 p-2 mt-8'>
					...
					<div>
						<div className='flex flex-col justify-center'>
							<button
								className='flex items-center gap-4 p-4 font-semibold text-surface-12 rounded-xl hover:bg-surface-3'>
					...
					<div>
						<div className='flex flex-col justify-center'>
							<button
								className='flex items-center gap-4 p-4 font-semibold text-surface-12 rounded-xl hover:bg-surface-3'>
								<p
									className='flex items-center h-4 gap-4 font-medium rounded-md aspect-square ring-2 ring-success-9'></p>
								<p>Sideprojects</p>
							</button>
						</div>
						<div className='flex flex-col justify-center'>
							<button
								className='flex items-center gap-4 p-4 font-semibold text-surface-12 rounded-xl hover:bg-surface-3'>
								<p
									className='flex items-center h-4 gap-4 font-medium rounded-md aspect-square ring-2 ring-primary-10'></p>
								<p className='flex items-center justify-between flex-1'>
...
function NavItem(
	...
	const router = useRouter();
	const isActive = props.active || router.pathname === props.href;
	return (
